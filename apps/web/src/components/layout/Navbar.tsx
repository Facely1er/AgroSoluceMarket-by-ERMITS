        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-gray-200 dark:border-gray-700 py-2">
            ...
            <div className="px-2 py-2">
              <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 mb-2">Main</div>
              {navLinks.filter(link => !link.secondary).map(({ to, icon: Icon, label, exact }) => {
                const active = exact ? isActive(to) : isActivePath(to);
                return (
                  <Link
            ...
            <div className="px-2 py-2 border-t border-gray-200 dark:border-gray-700 mt-2">
              <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 mb-2">More</div>
              {navLinks.filter(link => link.secondary).map(({ to, icon: Icon, label, exact }) => {
                const active = exact ? isActive(to) : isActivePath(to);
                return (
                  <Link
